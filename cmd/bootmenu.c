...
static struct bootmenu_data *bootmenu_create(int uefi, int delay)
{
	...
	if (i <= MAX_COUNT - 1) {
		...
		if (!IS_ENABLED(CONFIG_BOOTMENU_DISABLE_UBOOT_CONSOLE))
			entry->title = strdup("0. \033[1mExit\033[0m");
		else
			entry->title = strdup("0. \033[1mQuit\033[0m");
...
static void menu_display_statusline(struct menu *m)
{
	...
	puts("\033[1m   *** U-Boot Boot Menu ***\033[0m");
	...
	puts("\033[1mPress UP/DOWN to move, ENTER to select, ESC to quit\033[0m");
	puts(ANSI_CLEAR_LINE_TO_END);
