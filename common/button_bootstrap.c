...
static inline void
bootstrap_set_led_state(struct udevice *led, enum led_state_t state)
{
	if (led)
...
void button_bootstrap(void)
{
	...
	static const char *btn_propname[2] = { "boot-btn1", "boot-btn2" };
	static const char *led_propname[2] = { "boot-led1", "boot-led2" };
	...
	if (!strcmp(bootcmd, "initenv")) {
		env_set_default("Bootstrap: Initializing Environment...\n",
		...
		if (envcmd) {
			printf("Bootstrap: Running bootcmd '%s'...\n", bootcmd);
			run_command(envcmd, CMD_FLAG_ENV);
		} else {
			printf("Bootstrap: Environment variable '%s' not defined!\n", bootcmd);
		}
	}
}
